...
function DynaSidekick(){
	...
	self.SimpleQuery = function(conditions){
...
*/
	...
	self.SaveItem = function(dataModel, values, config){
		...
		for(var field in dataModel.data){
			var fieldModel = dataModel[field];

			if (field in values) {
				itemParams[fieldModel.dynamo.name] = self.DynamoUpdateFormatter(values[field], dataModel.dynamo.type);
			}
		}
		...
		for(var keyItem in dataModel.key) {
			var dynamoModel = dataModel.key[keyItem].dynamo;
			key[dynamoModel.name] = {};
			key[dynamoModel.name][dynamoModel.type] = values[keyItem];
		}
