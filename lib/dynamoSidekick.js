...
function DynaSidekick(){
	...
	self.DynamoUpdateFormatter = function(modelPropertyValue, dataType) {
		...
		var propValue = modelPropertyValue;
		var put = false;
		if (dataType === "S" && typeof propValue === 'string' && propValue.length > 0) put = true;
		if (dataType === "N" && typeof propValue === 'number') put = true;
		if ((dataType === "SS" || dataType === "NN") && propValue.isArray && propValue.length > 0) put = true;

		if (put) {
			if (dataType === "S" || dataType === "N") propValue = propValue.toString();
			property.Value = {};
	...
	self.SimpleScan = function(config, limit, start){
		var callback_ = arguments[arguments.length - 1];
		var callback = (typeof(callback_) == 'function' ? callback_ : function(){});

		var params = {'TableName': config.tableName, 'Limit': limit};
